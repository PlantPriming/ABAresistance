...
bestcp <- DTmodelext$cptable[which.min(DTmodelext$cptable[,"xerror"]),"CP"]
DTmodelext.pruned <- prune(DTmodelext, cp = bestcp)
...
set.seed(100)  
trainingRowIndex <- sample(1:nrow(RFext), 0.70*nrow(RFext))  # row indices for training data
trainingDataRFext <- RFext[trainingRowIndex, ]  # model training data
testDataRFext  <- RFext[-trainingRowIndex, ]   # test data
...
predicted.classes <- RFmodelext %>% predict(testDataRFext)
head(predicted.classes)
...
bestcp <- DTmodelage$cptable[which.min(DTmodelage$cptable[,"xerror"]),"CP"]
DTmodelage.pruned <- prune(DTmodelage, cp = bestcp)
